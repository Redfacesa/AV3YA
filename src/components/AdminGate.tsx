...
import type { Av3yaPlatformConfig } from '@/lib/platform-config';
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!isAllowedAdmin(next, config)) {
        await signOutAdmin();
        setError('This account is not authorised for AV3YA admin.');
        setSession(null);
        return;
      }
      setSession(next);
    ...
    return (
      <div className="pt-32 pb-16 section-padding max-w-lg mx-auto">
        ...
        <p className="text-xs text-white/30 mt-6 text-center">
          Linked merchant: {config.merchant?.business_name ?? 'AV3YA'}
    ...
    <div>
      <div className="fixed top-[4.5rem] right-3 sm:top-20 sm:right-4 z-40 flex items-center gap-2">
        <button
          type="button"
          onClick={onSignOut}
          className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 bg-av3ya-black/80 backdrop-blur-md text-white text-xs py-2.5 px-3 min-h-10 touch-manipulation hover:bg-white/10"
