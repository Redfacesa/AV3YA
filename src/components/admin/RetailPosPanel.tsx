  ...
  return (
    <div className={`space-y-4 ${mode === 'products' && cartCount > 0 ? 'pb-24 lg:pb-0' : ''}`}>
      ...
      {mode === 'products' && cartCount > 0 && (
        <div className="lg:hidden fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-av3ya-black/95 backdrop-blur-xl px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          <button
            type="button"
